...
use crate::{container::DEFAULT_DEVICES, log_child, pipe::sync::write_count, selinux};
...
pub fn parse_mount_table(mountinfo_path: &str) -> Result<Vec<Info>> {
    ...
    for line in reader.lines() {
        ...
        // |  |   |   |  |        |                           |          |     / source
        // |  |   |   |  |        |                           |          |     |      / vfs_opts
        // 22 96 0:21 / /sys rw,nosuid,nodev,noexec,relatime shared:2 - sysfs sysfs
        // rw,seclabel
...
pub fn ms_move_root(rootfs: &str) -> Result<bool> {
    ...
    for info in mount_infos.iter() {
        ...
        // Umount every syfs and proc file systems, except those under the container
        // rootfs
...
// This function constructs a canonicalized path by combining the `rootfs` and
// `unsafe_path` elements. The resulting path is guaranteed to be ("below" / "in
// a directory under") the `rootfs` directory.
//
// Parameters:
//
// - `rootfs` is the absolute path to the root of the containers root filesystem
//   directory.
// - `unsafe_path` is path inside a container. It is unsafe since it may try to
//   "escape" from the containers rootfs by using one or more "../" path
//   elements or is its a symlink to path.
...
fn mount_from(
    ...
    if !label.is_empty() {
        if selinux::is_enabled()? {
            ...
            log_child!(
                cfd_log,
                "SELinux label for the mount is provided but SELinux is not enabled on the \
                 running kernel"
            );
        }
    }
...
mod tests {
    ...
    fn test_parse_mount_table() {
        ...
        for (i, d) in tests.iter().enumerate() {
            ...
            let msg = format!("{}: No.{i}'s result: {:?}", msg, result);
